...
export default {
  ...
  async mounted() {
    ...
    const url = /(Android)/i.test(navigator.userAgent) ? window.location.href.split('#')[0] : window.entryUrl;
    const obj = {
      title: this.data.name, // 分享标题
      desc: this.data.createdAt, // 分享内容
      link: `${window.location.protocol}//${window.location.host}/job-detail?id=${this.$route.query.id}`, // 分享链接

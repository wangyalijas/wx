    ...
    <div class="job-resume" v-if="resume">
      ...
      <div class="job-resume__job">
        ...
        <div class="job-resume__education--button"
             @click="handleRouter('JobExperience')">+ 添加工作经历
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleGenderType(data) {
      ...
      return this.gender.filter(item => item.id === data)
        .shift().description;
    },
    openPicker(event) {
    ...
    getResume() {
      this.$indicator.open();
      this.$store.dispatch('resume/getResume')
        .then(() => {
    ...
    postResume() {
      this.$validator.validateAll()
        .then((result) => {
          ...
          this.$indicator.open();
          this.$store.dispatch('resume/postResume', this.resume.resume)
            .then((res) => {
              if (res.code === 200) {
                this.$toast(res.tip);
                this.$indicator.close();
